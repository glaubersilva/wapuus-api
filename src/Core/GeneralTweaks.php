...
namespace WapuusApi\Core;
	...
	class GeneralTweaks {
		...
		protected function init() {
			add_action( 'init', array( $this, 'imagesSize' ) );
			add_filter( 'rest_endpoints', array( $this, 'unsetEndpoints' ) );
			add_filter( 'rest_url_prefix', array( $this, 'changeApiPrefix' ) );
			add_filter( 'jwt_auth_expire', array( $this, 'changeAuthExpireToken' ) );
			add_action( 'rest_api_init', array( $this, 'maybeChangeCorsHeaders' ), 15 );
		}
		 ...
		 */
		public function unsetEndpoints( $endpoints ) {
		 ...
		 */
		public function changeApiPrefix( $prefix ) {
			$prefix = 'json';
			return $prefix;
		}
		 ...
		 */
		public function changeAuthExpireToken( $expire ) {
		 ...
		 */
		public function maybeChangeCorsHeaders() {
				...
				add_filter(
					...
					function( $value ) {
						...
						if ( $origin ) {
							...
							$allowedOrigins = array( 'wapuus.org', 'http://wapuus.org', 'https://wapuus.org', WAPUUS_API_RESTRICTED_MODE );
