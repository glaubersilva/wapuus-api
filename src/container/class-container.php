<?php
/**
 * Dependency injection container for the Wapuus API plugin.
 *
 * Based on the GiveWP Container:
 * @link https://github.com/impress-org/givewp/blob/develop/src/Container/Container.php
 *
 * @package Wapuus_API
...
namespace Wapuus_API\Src\Container;
...
class Container implements ArrayAccess //, ContainerInterface
{
    ...
    public function get(string $id)
    {
        try {
            ...
            throw new Exception(
                esc_html('Unable to resolve container entry: ' . $id),
                absint($e->getCode()),
                // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- $e is an Exception object (previous exception), not a string
                $e
            );
        }
    }
    ...
    public function build($concrete)
    {
        ...
        try {
            ...
            $concreteName = is_string($concrete) ? $concrete : 'Unknown';
            throw new Exception(
                esc_html('Target class [' . $concreteName . '] does not exist.'),
                0,
                // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- $e is an Exception object (previous exception), not a string
                $e
            );
    ...
    protected function unresolvablePrimitive(ReflectionParameter $parameter)
    {
        ...
        $message = 'Unresolvable dependency resolving [' . $parameterString . '] in class ' . $className;
        throw new Exception(esc_html($message));
    }
